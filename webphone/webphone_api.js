...
var parameters = {
                    serveraddress: '192.168.1.80', // your VoIP server IP address, domain name or SRV DNS record (also set the :port if that is not the default 5060)
...
{
    if ( !callback || typeof (callback) !== 'function' ) { return; }
    webphone_api.registerfailedcb = callback;
}
...
{
    if (typeof (plhandler) === 'undefined' || plhandler === null)
        api_helper.AddToQueue('RegisterEx', [accounts]);
    else
...
/** Initiate screenshare call to a number, sip username or SIP URI.*/
function screenshare (number)
{
    if (typeof (plhandler) === 'undefined' || plhandler === null)
        api_helper.AddToQueue('ScreenShare', [number]);
    else
        plhandler.ScreenShare(number);
}
