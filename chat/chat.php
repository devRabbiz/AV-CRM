...
class Model extends SPA_Common\Model
{
    ...
    {   
        $response = $this->db->query("(SELECT * FROM messages WHERE (username='{$_SESSION['login_username']}' AND to_username='{$to_username}' OR to_username='*') OR (username='{$to_username}'AND to_username='{$_SESSION['login_username']}' ) OR to_username='*'
