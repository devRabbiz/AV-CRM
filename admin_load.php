  ...
  $startrow = (int)$_GET['startrow'];
        ...
        // $r = "SELECT * FROM user WHERE sec='1'  AND reg_by='".$_SESSION['login_username']."' ORDER BY date DESC LIMIT $startrow, 30 ";
         ...
         //$q = "SELECT * FROM user WHERE sec='0' AND reg_by='".$_SESSION['login_username']."' ORDER BY date DESC LIMIT $startrow, 30 ";
          ...
          break;
          case 'operator':
            $r = mysqli_query($con,"SELECT * FROM user WHERE  lang='".$lang."' AND user.reg_by!='admin'  AND user.reg_by!='gabriele' AND user.reg_by!='adi' AND user.reg_by!='list'  ORDER BY date DESC LIMIT $startrow, 30 ");
            $o="class='active'";
            break;
            case 'op_leads':
...
<table id="etab1" class="table table-bordered table-hover" >
  <thead class="st">
          <tr >
